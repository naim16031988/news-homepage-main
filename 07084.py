...
def get_vm_to_scale_down():
    ...
    return jsonify({
        "vm_index_to_scale_down": max_index,
...
def get_q_table():
    return jsonify({str(k): v for k, v in q_table.items()})

@app.route('/reset_q_table', methods=['POST'])
def reset_q_table():
    global q_table, last_state, last_action
    q_table = defaultdict(float)
    last_state = None
    last_action = None
    return jsonify({'status': "Q-table reset successful."})
